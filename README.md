...
* Pøeveïte tento kód na webovou službu (ASP.NET Core Web API) a upravte kód tak aby bylo možné:
   ...
   * naèíst a uložit data z/do libovolné cesty na disku (pøípadnì cloud-storage)
   * naèíst data z HTTP URL (nelze ukládat)
   * odeslat výsledný soubor e-mailem (staèí pouze nástøel)
* Napište testy
* Refactorujte kód tak, jak by jste si pøedstavovali produkèní aplikaci, která je vyvíjena a provozována vaším týmem.
